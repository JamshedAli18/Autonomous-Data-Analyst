...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        if analyze_button:
            with st.spinner("Analyzing your data... This may take 1-2 minutes."):
                try:
                    ...
                    st.info("Please try with a different CSV file or check the file format.")
        ...
        if st.session_state.analysis_result:
            ...
            with col1:
                data_shape = result.get("data_shape", "N/A")
                st.metric("Data Shape", data_shape)
            
            with col2:
                charts_count = result.get("charts_generated", 0)
                st.metric("Charts Generated", charts_count)
            
            with col3:
                columns_count = len(result.get("columns", result.get("summary", {}).get("columns", [])))
...
with tab2:
    if st.session_state.analysis_result:
        ...
        if "generated_code" in result and result["generated_code"]:
            ...
            st.warning("Code generation encountered an issue. Please try analyzing again.")
        ...
        if charts_count > 0 and "chart_files" in result:
            ...
            for idx, (chart_file, title, explanation) in enumerate(
                ...
                try:
                    ...
                    st.warning(f"Could not display chart {idx}: {title}")
        
        elif charts_count == 0:
            st.info("No charts were generated for this dataset. This may happen with very small or single-column datasets.")
        else:
            st.warning("Chart data not available. Please try analyzing again.")
